...
import static com.github.razorplay01.geoware.donkeykong.game.util.records.Hitbox.HITBOX_BARREL;
import static com.github.razorplay01.geoware.donkeykong.game.util.records.Hitbox.HITBOX_LADDER;
...
public class PlayerCollisionHandler {
    ...
    public static CollisionResult handleBarrelCollision(Player player, List<Barrel> barrels) {
        ...
        return barrels.stream()
                .map(barrel -> checkSingleBarrelCollision(player, barrel, playerHitbox))
    ...
    private static CollisionResult checkSingleBarrelCollision(Player player, Barrel barrel, Hitbox playerHitbox) {
        ...
        BarrelCollisionData collisionData = new BarrelCollisionData(
                playerHitbox.yPos() + playerHitbox.height() <= barrelHitbox.yPos() + BARREL_TOP_OFFSET,
                isXOverlapping(playerHitbox, barrelHitbox),
                Math.abs(barrelHitbox.yPos() - (playerHitbox.yPos() + playerHitbox.height())) < BARREL_JUMP_HEIGHT
        );

        return evaluateBarrelCollision(player, barrel, playerHitbox, barrelHitbox, collisionData);
    }

    private static boolean isXOverlapping(Hitbox a, Hitbox b) {
