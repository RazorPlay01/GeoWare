package com.github.razorplay01.geoware.tetris.network;
...
import com.github.razorplay01.geoware.tetris.Tetris;
import com.github.razorplay01.geoware.tetris.TetrisGameScreen;
...
public class NetworkManager {
    ...
    public static void registerClient() {
        CLIENT_HANDLERS.put("TetrisPacket", NetworkManager::checkArkanoidPacketClient);
        ...
        ClientPlayNetworking.registerGlobalReceiver(FabricCustomPayload.CUSTOM_PAYLOAD_ID, (payload, context) -> {
            ...
            Tetris.LOGGER.info("Packet received from server: {}", packet.getPacketId());
            BiConsumer<IPacket, ClientPlayNetworking.Context> handler = CLIENT_HANDLERS.getOrDefault(packet.getPacketId(), (p, c) -> Tetris.LOGGER.info("Unknown server packet: {}", p.getPacketId()));
    ...
    private static void checkArkanoidPacketClient(IPacket iPacket, ClientPlayNetworking.Context context) {
        TetrisPacket pkt = (TetrisPacket) iPacket;
        context.client().execute(() -> MinecraftClient.getInstance().setScreen(new TetrisGameScreen(pkt.getScore(), 5, pkt.getTimeLimitSeconds(), pkt.getSpeedMultiplier())));
    }
    ...
    public static void registerServer() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(FabricCustomPayload.CUSTOM_PAYLOAD_ID, (payload, context) -> {
            IPacket packet = payload.packet();
            Tetris.LOGGER.info("Packet received from client: {}", packet.getPacketId());
            BiConsumer<IPacket, ServerPlayNetworking.Context> handler = SERVER_HANDLERS.getOrDefault(packet.getPacketId(), (p, c) -> Tetris.LOGGER.info("Unknown client packet: {}", p.getPacketId()));
            handler.accept(packet, context);
        });
    }
}