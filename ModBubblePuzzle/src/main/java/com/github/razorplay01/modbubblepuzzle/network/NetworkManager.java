package com.github.razorplay01.modbubblepuzzle.network;
...
public class NetworkManager {
    ...
    public static void registerClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(FabricCustomPayload.CUSTOM_PAYLOAD_ID, (payload, context) -> {
            ...
            ModBubblePuzzle.LOGGER.info("Packet received from server: {}", packet.getPacketId());
            BiConsumer<IPacket, ClientPlayNetworking.Context> handler = CLIENT_HANDLERS.getOrDefault(packet.getPacketId(), (p, c) -> ModBubblePuzzle.LOGGER.info("Unknown server packet: {}", p.getPacketId()));
    ...
    private static void checkArkanoidPacketClient(IPacket iPacket, ClientPlayNetworking.Context context) {
        BubblePuzzlePacket pkt = (BubblePuzzlePacket) iPacket;
        context.client().execute(() -> MinecraftClient.getInstance().setScreen(new BubblePuzzleScreen(pkt.getScore(), 5, pkt.getTimeLimitSeconds(), pkt.getLevel())));
    }
    ...
    public static void registerServer() {
        ServerPlayNetworking.registerGlobalReceiver(FabricCustomPayload.CUSTOM_PAYLOAD_ID, (payload, context) -> {
            ...
            BiConsumer<IPacket, ServerPlayNetworking.Context> handler = SERVER_HANDLERS.getOrDefault(packet.getPacketId(), (p, c) -> ModBubblePuzzle.LOGGER.info("Unknown client packet: {}", p.getPacketId()));
            handler.accept(packet, context);
        });
    }
}